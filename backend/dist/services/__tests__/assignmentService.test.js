...
describe('Assignment Service', () => {
    ...
    describe('createAssignment', () => {
        it('should create an assignment for instructor-owned course', async () => {
            let instructor, course, assignment;
            try {
                // Create instructor
                ...
                // Clean up
                if (assignment) {
                    await prisma_1.prisma.assignment.delete({ where: { id: assignment.id } });
                }
                if (course) {
                    await prisma_1.prisma.course.delete({ where: { id: course.id } });
                }
                if (instructor) {
                    await prisma_1.prisma.user.delete({ where: { id: instructor.id } });
                }
            }
        });
        it('should reject assignment creation for non-owned course', async () => {
            let instructor1, instructor2, course;
            try {
                // Create first instructor
                const hashedPassword1 = await (0, password_1.hashPassword)('TestPass123');
                instructor1 = await prisma_1.prisma.user.create({
                    data: {
                ...
                const hashedPassword2 = await (0, password_1.hashPassword)('TestPass123');
                instructor2 = await prisma_1.prisma.user.create({
                    data: {
                ...
                await expect(assignmentService.createAssignment(assignmentData, instructor2.id)).rejects.toThrow('You do not have permission to create assignments for this course');
            }
            finally {
                // Clean up
                if (course) {
                ...
                if (instructor1) {
                    await prisma_1.prisma.user.delete({ where: { id: instructor1.id } });
                }
                if (instructor2) {
                    await prisma_1.prisma.user.delete({ where: { id: instructor2.id } });
                }
            }
        });
    });
    describe('getAssignmentsByCourse', () => {
        it('should return assignments for enrolled student', async () => {
            let instructor, student, course, assignment;
            try {
                // Create instructor
                ...
                const assignments = await assignmentService.getAssignmentsByCourse(course.id, student.id, 'student');
        ...
        it('should reject access for non-enrolled student', async () => {
            ...
            try {
                ...
                await expect(assignmentService.getAssignmentsByCourse(course.id, student.id, 'student')).rejects.toThrow('You are not enrolled in this course');
            }
            finally {
                // Clean up
                if (course) {
...
//# sourceMappingURL=assignmentService.test.js.map