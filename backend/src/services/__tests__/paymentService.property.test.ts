  ...
  describe('Property 3: Payment Completion Grants Access', () => {
    it('should grant immediate access to paid course after successful payment completion', async () => {
      ...
      for (const testCase of testCases) {
        let instructor: any, student: any, course: any, payment: any;
        ...
        try {
          ...
          const createdInstructor = await prisma.user.findUnique({
          ...
          payment = await prisma.payment.create({
            data: {
              ...
              const enrollment = await PaymentService.enrollStudentAfterPayment(payment.id);
          ...
          try {
            await courseService.enrollStudentWithPaymentVerification(
              course.id, 
              student.id, 
              payment.id
            );
    ...
    it('should reject enrollment with incomplete payment', async () => {
      ...
      try {
        ...
        try {
          await PaymentService.enrollStudentAfterPayment(payment.id);
    ...
    it('should reject enrollment with payment for different course', async () => {
      let instructor: any, student: any, course1: any, course2: any, payment: any;
      ...
      try {
        ...
        const correctEnrollment = await courseService.enrollStudentWithPaymentVerification(
          course1.id, 
          student.id, 
          payment.id
        );
