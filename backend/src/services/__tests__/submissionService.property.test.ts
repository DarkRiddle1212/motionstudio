  ...
  describe('Property 7: Submission Persistence', () => {
    it('should persist and retrieve submission data consistently for file submissions', async () => {
      ...
      for (const testCase of testCases) {
        let instructor: any, student: any, course: any, assignment: any, submission: any;
        ...
        try {
          ...
          const retrievedSubmission = await submissionService.getSubmissionById(
            submission.id, 
            student.id, 
            'student'
          );
          ...
          expect(retrievedSubmission.submittedAt.getTime()).toBeCloseTo(
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ beforeDeadlineMinutes, afterDeadlineMinutes, submissionType }) => {
            let instructor: any, student: any, course: any, assignmentBefore: any, assignmentAfter: any;
            let submissionBefore: any, submissionAfter: any;
            ...
            try {
              ...
              const retrievedBefore = await submissionService.getSubmissionById(
                submissionBefore.id, 
                student.id, 
                'student'
              );
              const retrievedAfter = await submissionService.getSubmissionById(
                submissionAfter.id, 
                student.id, 
                'student'
              );
